...
const Employee = () => {
  ...
  const handleDelete = (id) => {
    axios.delete(`${apiUrl}/delete_employee/` + id).then((result) => {
      if (result.data.Status) {
  ...
  return (
    <div className="w-full pt-6 px-6 mb-8">
      <div className="card bg-white rounded-xl shadow-md ">
        <h2 className="text-4xl font-montserrat text-center py-3 text-gray-800 ">
          Employee List
        </h2>
        ...
        <div className="font-montserrat font-bold py-3 mb-4">
          <p className="bg-gray-800 px-2 w-36 rounded mx-3 text-white">
            <span className="font-montserrat">Total Assets:</span>
            <br />
            Rs. {salaryTotal}
          </p>
        </div>
            ...
            <table className="min-w-full text-left border-collapse hidden sm:table">
              ...
              <tbody>
                {employees.map((employee) => (
                  <tr
                    key={employee._id}
                    className="hover:bg-gray-100 border-b transition-colors duration-200"
                  >
