...
const EmpDashboard = () => {
  ...
  return (
      ...
      <div className="flex flex-col items-center md:items-start">
        ...
        <ul className="flex flex-col space-y-8 px-0 list-none">
          <li>
            <Link
              to={`employee_detail/${id}`}
              className="flex items-center text-white no-underline px-2 py-1 hover:bg-gray-700 rounded"
            >
          ...
          <li>
            <Link
              to={`EventSchedule/${id}`}
              className="flex items-center no-underline text-white px-2 py-1 hover:bg-gray-700 rounded"
            >
               <span className="px-2 text-2xl ">
                  <MdOutlineEventAvailable />
                </span>
              <span className="hidden md:inline no-underline">Event</span>
            </Link>
          </li>
          <li>
            <Link
              to={`TrainingModules/${id}`}
              className="flex items-center text-white px-2 py-1 no-underline hover:bg-gray-700 rounded"
            >
    ...
    <main className="flex-grow w-3/4 p-4 relative bg-logo-gradient">
       ...
       {/* Conditionally render the Welcome message only on EmpDashboard */}

       {location.pathname.includes(`/EmpDashboard/employee_detail/${id}`) && (
